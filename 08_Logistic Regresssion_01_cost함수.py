'''
파이썬으로 Logistic Regression 직접 구현

제목을 붙일 게 없어서 직접 구현한다고는 했지만, 거짓말이다.
직접 구현하려면 cost 함수와 gradient descent 알고리즘의 2개를 모두 만들어야 하는데,
여기서는 cost 함수만 구현했다. 왜 그랬는지는 차차 알아보자.
